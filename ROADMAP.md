...
- [x] **Extension published July 21 2026:** `EdwardNg.vlint v0.1.1` is live on the VS Code Marketplace (`./build.sh` then `vsce publish --packagePath vlint.vsix -p <PAT>`). The Marketplace PAT comes from Azure DevOps (scope Marketplace → Manage, "all accessible organizations"), separate from npm and deliberately not in CI. 0.1.1 rather than 0.1.0 because a stale 0.1.0 (pre-Connect-Figma) was already published July 17 and Marketplace versions are immutable. Publish the prebuilt vsix, not a bare `vsce publish`, since `vscode:prepublish` is a stub and `build.sh` does the real bundling.
